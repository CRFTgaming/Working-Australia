...
_kassa = 15000 + round(random 5000);
...
if(side _robber != civilian) exitWith {
  hint "You can not rob this manager!"
};

if(_robber distance _shop > 12.5) exitWith {
  hint "You need to be within 3.5m of the manager to rob him!"
};
...
if (_shop getVariable "robbery_in_progress") exitWith {
  hint "Manager is already being robbed!";
};
...
_chance = random(100);
if(_chance >= 80) then {
  hint "The manager hit the silent alarm, police have been alerted!";
...
_pgText ctrlSetText format["Robbery in Progress, stay close (12.5m) (1%1)...","%"];
...
while{true} do {
  ...
  _pgText ctrlSetText format["Robbery in Progress, stay close (12.5m) (%1%2)...",round(_cP * 100),"%"];

  if(_cP >= 1) exitWith {};
  if(_robber distance _shop > 12.5) exitWith { _abort = true; };
  if!(alive _robber) exitWith {};
};
...
if(_abort) exitWith {
  _shop switchMove "";
  hint "You need to stay within 3.5m to rob the manager! - Now the safe is locked.";
  5 cutText ["","PLAIN"];
};
5 cutText ["","PLAIN"];
