...
if(side _robber != civilian) exitWith {
  hint "You can not rob this teller!"
};

if(_robber distance _shop > 3.5) exitWith {
  hint "You need to be within 3.5m of the teller to rob him!"
};
...
if (_shop getVariable "robbery_in_progress") exitWith {
  hint "Teller is already being robbed!";
};
...
_chance = random(100);
if(_chance >= 60) then {
  hint "The teller hit the silent alarm, police have been alerted!";
...
if(_abort) exitWith {
  _shop switchMove "";
  hint "You need to stay within 3.5m to rob the teller! - Now the register is locked.";
  5 cutText ["","PLAIN"];
};
5 cutText ["","PLAIN"];
