...
_vehicleList = [life_veh_shop select 0] call life_fnc_vehicleListCfg; _basePrice = (_vehicleList select _vIndex) select 1;
 if(_mode) then {_basePrice = round(_basePrice * 15)};
...
switch(playerSide) do {
	...
	case civilian: {
		if((life_veh_shop select 2) == "civ" && {_className == "C_Heli_Light_01_civil_F"}) then {
			[_vehicle,"civ_littlebird",true] spawn life_fnc_vehicleAnimate;
		};

		if((life_veh_shop select 2) == "civ" && {_className == "C_Offroad_01_repair_F"}) then {
